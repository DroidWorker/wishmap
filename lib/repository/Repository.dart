...
class Repository {
  ...
  Future<String?> registration(ProfileData pd, AuthData ad) async {
    try {
      await _auth.createUserWithEmailAndPassword(
          email: ad.login, password: ad.password);
      if (_auth.currentUser != null) {
  ...
  Future<int?> getLastImageId() async {
    if (_auth.currentUser != null) {
      DataSnapshot snapshot =
          (await userRef.child(_auth.currentUser!.uid).child("images").once())
              .snapshot;
  ...
  Future<Map<int, String>> fetchImages(int id, int moonId) async {
    ...
    if (snapshot.value != null) {
      for (var element in snapshot.children) {
        if (element.key != null)
          photos[int.parse(element.key!)] = element.value.toString();
      }
      return photos;
    }
  ...
  Future<String> saveFile(File file) async {
    Reference ref = storageRef.child(file.path.split("/").last);
  ...
  Future<List<MoonItem>?> getMoonList() async {
    if (_auth.currentUser != null) {
      DataSnapshot snapshot =
          (await userRef.child(_auth.currentUser!.uid).child("moonlist").once())
              .snapshot;
      if (snapshot.children.isNotEmpty) {
        List<MoonItem> moons = [];
        snapshot.children.forEach((element) {
          if (element.key != null && element.value != null) {
            ...
            moons.add(
                MoonItem(id: id, filling: filling, text: text, date: date));
          }
        });
        return moons;
      } else {
  ...
  Future addMoon(MoonItem moonItem, List<CircleData>? defaultCircles,
      List<WishData>? defaultWishes) async {
    if (_auth.currentUser != null) {
      await userRef
          .child(_auth.currentUser!.uid)
          .child("moonlist")
          .child(moonItem.id.toString())
          .set({
        'text': moonItem.text,
        'date': moonItem.date,
  ...
  Future addDiary(List<CardData> data) async {
    if (_auth.currentUser != null) {
      if (data.length > 1) {
        ...
        userRef.child(_auth.currentUser!.uid).child("diary").set(indexedDiary);
      } else {
        Map<String, dynamic> dataMap = {
          'id': data.last.id,
  ...
  Future<Map<CardData, List<Article>>?> getDiaryList() async {
    if (_auth.currentUser != null) {
      ...
      DataSnapshot dataSnapshot =
          (await userRef.child(_auth.currentUser!.uid).child("diary").once())
              .snapshot;
      if (dataSnapshot.children.isNotEmpty) {
        dataSnapshot.children.forEach((element) {
    ...
    if (_auth.currentUser != null) {
      ...
      if (dataSnapshot.value != null && dataSnapshot.key != null) {
        ...
        final date = DateFormat("dd.MM.yyyy")
            .parse(dataList["expiryDate"])
            .add(const Duration(days: 1));
        return Promocode(dataSnapshot.key!,
            DateFormat("dd.MM.yyyy").format(date), dataList['type']);
      }
    }
    return null;
  }
