...
class _WishScreenState extends State<WishScreen> {
  ...
  initState() {
    ...
    _scrollController.addListener(() {
      MyTreeViewState? treeViewState =
          _treeViewKey.currentState as MyTreeViewState;

      RenderBox? renderBox =
          _keyToScroll.currentContext?.findRenderObject() as RenderBox?;
  ...
  Widget build(BuildContext context) {
    ...
    return Consumer<AppViewModel>(builder: (context, appVM, child) {
      if (appVM.wishScreenState != null && curwish.id == -1) {
        ...
        final parentObj = appVM.mainScreenState!.allCircles
            .where((element) => element.id == curwish.parentId)
            .firstOrNull;
        isParentChecked = parentObj?.isChecked ?? true;
        isParentActive = parentObj?.isActive ?? false;
        isParentHidden = parentObj?.isHidden ?? true;
        parentIsSphere =
            parentObj != null ? (parentObj.parenId <= 0 ? true : false) : false;
      ...
      WidgetsBinding.instance.addPostFrameCallback((timeStamp) {
        if (appVM.needAutoScrollBottom) {
          final RenderObject? renderObject =
              _keyToScroll.currentContext?.findRenderObject();
            ...
            child: Column(children: [
                  ...
                  children: [
                    IconButton(
                        ...
                        icon: const Icon(Icons.keyboard_arrow_left,
                            size: 32, color: AppColors.gradientStart),
                        onPressed: () {
                          if (curwish.isHidden) {
                            appVM.mainCircles.clear();
                            appViewModel
                                .startMainScreen(appVM.mainScreenState!.moon);
                          ...
                          if (!curwish.isChecked && appViewModel.isChanged) {
                            showModalBottomSheet<void>(
                              ...
                              builder: (BuildContext context) {
                                return ActionBS(
                                    'Внимание',
                                    "Вы изменили поля но не нажали 'Сохранить'\nСохранить изменения?",
                                    "Да",
                                    'Нет', onOk: () async {
                                  Navigator.pop(context, 'OK');
                                  onSaveClicked(appVM, true, _title,
                                      _description, _affirmation);
                                }, onCancel: () {
                          ...
                          if (appViewModel.backPressedCount ==
                              appViewModel.settings.quoteupdateFreq) {
                      ...
                      onPressed: () async {
                        showModalBottomSheet<void>(
                          ...
                          builder: (BuildContext context) {
                            return ActionBS(
                                'Внимание',
                                (curwish.childAims.isEmpty &&
                                        !appVM.hasChildWishes(curwish.id))
                                    ? "Объект будет удален"
                                    : (curwish.parentId > 1)
              ...
              Expanded(
                ...
                child: Padding(
                  ...
                  child: Column(
                    children: [
                      !curwish.isActive && !curwish.isChecked
                          ? ColorRoundedButton(
                              "Представить",
                              () {
                                if (appVM.settings.actualizeFullBranch) {
                                  ...
                                  setState(() {
                                    appViewModel.mainCircles
                                        .where((element) =>
                                            element.id == curwish.id)
                                        .firstOrNull
                                        ?.isActive = true;
                                    curwish.isActive = true;
                                  });
                                  if (curwish.parentId == 0) {
                                    if (appVM.settings.animationEnabled)
                                      showOverlayedAnimations(context,
                                          'assets/lottie/aktualizaciyasfery.json',
                                          fillBackground: true);
                                  } else {
                                    if (appVM.settings.animationEnabled)
                                      showOverlayedAnimations(context,
                                          'assets/lottie/aktualizaciyazhelaniya.json',
                                          fillBackground: true);
                                  }
                                  return;
                                }
                                if (isParentActive ||
                                    appVM.settings.wishActualizingMode == 1 ||
                                    parentIsSphere) {
                                  if (appVM.settings.sphereActualizingMode ==
                                          1 ||
                                      appVM.isParentSphereActive(curwish.id) ||
                                      curwish.parentId == 0) {
                                    appViewModel.activateSphereWish(
                                        curwish.id, true);
                                    setState(() {
                                      appViewModel.mainCircles
                                          .where((element) =>
                                    ...
                                    showUnavailable(
                                        "Чтобы представить это желание необходимо сначала представить вышестоящиую сферу");
                                  }
                                } else {
                                  showUnavailable(
                                      "Чтобы представить это желание необходимо сначала представить вышестоящий объект");
                                }
                              },
                            )
                          : const SizedBox(),
                      if (curwish.parentId > 1 &&
                          curwish.isActive &&
                          !curwish.text.contains("HEADERSIMPLETASKHEADER"))
                        Row(children: [
                          Expanded(
                            child: OutlinedGradientButton("Исполнено",
                                filledButtonColor: curwish.isChecked
                                    ? AppColors.greenButtonBack
                                    : null, () async {
                              if (isParentChecked) {
                                showCantChangeStatus();
                              } else if (curwish.isHidden) {
                                showUnavailable(
                                    "Чтобы изменить статус 'исполнено' необходимо отменить статус 'скрыто'");
                              } else {
                                if (appVM.isChanged) {
                                  showModalBottomSheet<void>(
                                    backgroundColor: AppColors.backgroundColor,
                                    ...
                                    builder: (BuildContext mcontext) {
                                      return ActionBS(
                                          ...
                                          'Нет', onOk: () async {
                                        Navigator.pop(context, 'OK');
                                        final result = await onSaveClicked(
                                            appVM,
                                            false,
                                            _title,
                                            _description,
                                        ...
                                        curwish = WishData(
                                            ...
                                            color: Colors.red);
                                        final retWish =
                                            await appVM.startWishScreen(
                                                wishid, curwish.parentId, false,
                                                isUpdateScreen: true);
                                        curwish = retWish;
                                        changeStatus(appVM);
                                      });
                              ...
                              child: Center(
                                child: IconButton(
                                  ...
                                  onPressed: () async {
                                    if (isParentHidden) {
                                      showUnavailable(
                                          "Чтобы изменить статус необходимо отменить статус 'скрыто' для вышестоящего желания");
                                    } else if (curwish.isHidden &&
                                        appVM.getShowedCirclesCount(
                                                curwish.parentId) >=
                                            12) {
                                      showModalBottomSheet<void>(
                                        backgroundColor:
                                        ...
                                        builder: (BuildContext context) {
                                          return NotifyBS(
                                              'Внимание',
                                              "Превышено количество 'Желаний на орбите'. Максимальное количество желаний на орбите 12 шт. Вы можете скрыть или удалить другие желания, чтобы освободить место для демонстрации данного желания на орбите",
                                              'OK',
                                              onOk: () =>
                                                  Navigator.pop(context, 'OK'));
                                        },
                                          ...
                                          return ActionBS(
                                              ...
                                              curwish.childAims.isEmpty &&
                                                  ...
                                                  ? (curwish.isHidden
                                                      ...
                                                      : "Скрыть?")
                                                  : curwish.isHidden
                                                      ? "Если в данном желании создавались другие желания то они не будут отображены. Отобразить?"
                                                      : "Если в данном желании создавались другие желания, цели и задачи, то они также будут скрыты. Скрыть?",
                                              "Да",
                                              'Нет', onOk: () {
                                            Navigator.pop(context, 'OK');
                                            setState(() {
                        ...
                        Column(children: [
                          TextField(
                            ...
                            onTap: () {
                              if (curwish.isChecked && !curwish.isActive) {
                                showUnavailable(
                                    "Желание исполнено в прошлой карте. Изменению не подлежит. Вы можете видеть ее в журнале желаний в разделах 'исполненные' и 'все цели', а также в иерархии желания.\n\nВы можете удалить желание. Если вам нужна подобная, просто создайте новую.");
                              } else if (curwish.text
                                  .contains("HEADERSIMPLETASKHEADER")) {
                                showUnavailable(
                                    "Это желание содержит простые задачи. Его невозможно изменить");
                              } else if (curwish.isChecked)
                                showUnavailable(
                          ...
                          TextField(
                            ...
                            onTap: () async {
                              if (curwish.isChecked) {
                                showUnavailable(
                                    "Чтобы редактировать желание необходимо сменить статус \nна 'не выполнено'");
                                return;
                                ...
                                final affirmationsStr = curwish.affirmation ==
                                    ...
                                    : await showOverlayedAffirmations(
                                        context,
                                        appVM.wishScreenState!.wish.affirmation
                                            .split("|"),
                                        true,
                                        curwish.shuffle,
                                        onShuffleClick: (value) {
                          ...
                          TextField(
                            ...
                            onTap: () async {
                              if (curwish.isChecked) {
                                ...
                                final newText = await showOverlayedEdittext(
                                        context,
                                        _description.text,
                                        (curwish.isActive &&
                                            !curwish.isChecked)) ??
                                    "";
                                if (newText != _description.text)
                                  appViewModel.isChanged = true;
                                setState(() {
                                ...
                                return Card(
                                  ...
                                  child: InkWell(
                                    ...
                                    child: SizedBox(
                                      width: 34.0,
                                      height: 34.0,
                                      child: _color == defaultColorList[index]
                                          ? const Icon(
                                              Icons.check,
                                              color: Colors.white,
                                              size: 20,
                          ...
                          if (curwish.parentId > 1 &&
                              curwish.isActive &&
                              !curwish.isChecked)
                            OutlinedGradientButton("Добавить цель", () {
                              if (curwish.isChecked && !curwish.isActive) {
                                showUnavailable(
                                    "Желание исполнено в прошлой карте. Изменению не подлежит. Вы можете видеть ее в журнале желаний в разделах 'исполненные' и 'все цели', а также в иерархии желания.\n\nВы можете удалить желание. Если вам нужна подобная, просто создайте новую.");
                              } else if (curwish.isChecked)
                                ...
                                if (appVM.mainScreenState!.allCircles
                                  ...
                                  BlocProvider.of<NavigationBloc>(context).add(
                                      NavigateToAimCreateScreenEvent(
                                          appVM.wishScreenState!.wish.id));
                                } else {
                                  showModalBottomSheet<void>(
                                    backgroundColor: AppColors.backgroundColor,
                          ...
                          const SizedBox(height: 8),
                          if (curwish.isActive && !curwish.isChecked)
                            OutlinedGradientButton(
                              ...
                              () async {
                                if (appVM.mainScreenState!.allCircles
                                    ...
                                    12) {
                                  showUnavailable(
                                      "Достигнуто максимальноке количество желаний на орбите. Вы можете скрть или удалить другие желания, чтобы освободить место для демонстрации данной желания на орбите");
                                } else {
                                  appVM.cachedImages.clear();
                                  final childlastid = appViewModel
                                          ...
                                          ?.id ??
                                      -1;
                                  int wishid = appViewModel.mainScreenState!
                                          .allCircles.isNotEmpty
                                      ? appViewModel.mainScreenState!.allCircles
                                  ...
                                  await appViewModel.createNewSphereWish(
                                      WishData(
                                          ...
                                          text: "Новое желание",
                                          description: "",
                                          affirmation: (defaultAffirmations
                                              .join("|")
                                              .toString()),
                                          color: Colors.red),
                                      true,
                          ...
                          const SizedBox(height: 8),
                          if (curwish.isActive && !curwish.isChecked)
                            OutlinedGradientButton("Создать общую задачу", () {
                              if (curwish.isChecked && !curwish.isActive) {
                                showUnavailable(
                                    "Желание исполнено в прошлой карте. Изменению не подлежит. Вы можете видеть ее в журнале желаний в разделах 'исполненные' и 'все цели', а также в иерархии желания.\n\nВы можете удалить желание. Если вам нужна подобная, просто создайте новую.");
                              } else if (curwish.isChecked)
                                ...
                                if (appVM.mainScreenState!.allCircles
                                  ...
                                  appVM.myNodes.clear();
                                  BlocProvider.of<NavigationBloc>(context).add(
                                      NavigateToTaskCreateScreenEvent(
                                          curwish.id,
                                          isSimple: true,
                                          type: curwish.parentId > 1
                                              ? 'w'
                                              : 's'));
                                } else {
                                  showModalBottomSheet<void>(
                            ...
                            children: [
                              ...
                              appVM.settings.treeView == 0
                                  ? MyTreeView(
                                      key: _treeViewKey,
                                      roots: root,
                                      currentId: curwish.id,
                                      colors: appVM.nodesColors,
                                      onTap: (id, type) => onTreeItemTap(
                                          appVM,
                                          id,
                                          type,
                                          _title,
                                  ...
                                  () async {
                                    ...
                                    if (_title.text.isEmpty ||
                                      ...
                                      await showModalBottomSheet<void>(
                                        ...
                                        builder: (BuildContext context) {
                                          return NotifyBS(
                                              'Необходимо заполнить все поля со знаком *',
                                              "",
                                              'OK',
                                              onOk: () =>
                                                  Navigator.pop(context, 'OK'));
                      ...
                      children: [
                        if (!curwish.text.contains("HEADERSI")&&!curwish.isHidden)
                          FloatingActionButton(
                            ...
                            backgroundColor: curwish.isActive
                                ? curwish.color
        ...
        bottomNavigationBar: BottomBar(
          ...
          onMapTap: () {
            ...
            if (pressNum > 5) {
              ...
              appVM.hint =
                  "Кнопка “карта” возвращает вас на верхний уровень карты “желаний”. Сейчас вы уже здесь!";
            }
  ...
  void changeStatus(AppViewModel appVM) {
    if (appVM.wishScreenState!.wish.childAims.isNotEmpty ||
        appVM.mainScreenState?.allCircles.firstWhereOrNull((element) =>
                element.parenId == appVM.wishScreenState!.wish.id) !=
            null) {
      showModalBottomSheet<void>(
        backgroundColor: AppColors.backgroundColor,
        context: context,
        ...
        builder: (BuildContext context) {
          return ActionBS(
              'Внимание',
              (!curwish.isChecked)
                  ? "Если в данном желании создавались другие желания, цели и задачи, то они также получат статус 'исполнена' / 'достигнута' / 'выполнена'. Исполнено?"
                  : "Если в данном желании создавались другие желания, цели и задачи, то они останутся в статусе 'исполнена' / 'достигнута' / 'выполнена'. Не исполнено?",
              "Да",
              'Нет', onOk: () {
            Navigator.pop(context, 'OK');
            curwish.isChecked = !curwish.isChecked;
            if (curwish.isChecked && appVM.settings.animationEnabled)
              showOverlayedAnimations(
                  context, 'assets/lottie/vypolneniezhelaniya.json',
                  fillBackground: true, onControllerCreated: (controller) {
                lottieController = controller;
              });
            appVM.updateWishStatus(
                appVM.wishScreenState!.wish.id, curwish.isChecked);
            showModalBottomSheet<void>(
              backgroundColor: AppColors.backgroundColor,
              context: context,
              isScrollControlled: true,
              builder: (BuildContext context) {
                return NotifyBS(
                    curwish.isChecked ? 'исполнено' : 'не исполнено', "", 'OK',
                    onOk: () {
                  try {
      ...
      appVM.updateWishStatus(appVM.wishScreenState!.wish.id, curwish.isChecked);
      showModalBottomSheet<void>(
        backgroundColor: AppColors.backgroundColor,
        context: context,
        isScrollControlled: true,
  ...
  Future<bool> onSaveClicked(
      AppViewModel appVM,
      bool isExit,
      TextEditingController title,
      TextEditingController description,
      TextEditingController affirmation) async {
    ...
    showModalBottomSheet<void>(
      ...
      isScrollControlled: true,
      builder: (BuildContext context) {
        return NotifyBS('Сохранено', "", 'OK', onOk: () {
          Navigator.pop(context, 'OK');
          if (isExit) {
  ...
  void showCantChangeStatus() {
    showModalBottomSheet<void>(
      ...
      builder: (BuildContext context) {
        return NotifyBS(
            'Внимание',
            "Статус желания не может быть изменен на 'не исполнено' пока вышестоящее желание не будет переведено в статус 'не исполнено'",
            'OK',
            onOk: () => Navigator.pop(context, 'OK'));
      },
    );
  ...
  Future onTreeItemTap(
    ...
    if (type == "m") {
      if (appVM.isChanged) {
        if (await showOnExit(appVM, title, description, affirmation) == false)
          return;
      }
