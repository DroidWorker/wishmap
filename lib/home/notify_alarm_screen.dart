...
class NotifyAlarmScreenState extends State<NotifyAlarmScreen> {
  ...
  Widget build(BuildContext context) {
    if (cyrrepeatInterval != null &&
        cyrrepeatInterval!.millisecond > DateTime.now().millisecond) {
    ...
    if (alarm == null) {
      appViewModel.getAlarmById(widget.alarmId).then((v) {
        setState(() {
          alarm = v;
          if (v != null) {
            ...
            body: Padding(
              ...
              child: Column(
                ...
                children: [
                  ...
                  Text(
                    "${fullDayOfWeek[current.weekday]}, ${current.day} ${monthOfYear[current.month]} ${current.year}",
                    style: const TextStyle(color: Colors.white),
                  ),
                  Text(DateFormat('HH:mm').format(current),
                      style:
                  ...
                  Text(
                      IMenAffirmations[
                          Random().nextInt(IMenAffirmations.length)],
                      ...
                      child: Column(
                        ...
                        children: [
                          ...
                          SvgPicture.asset('assets/icons/ring.svg',
                              height: 32, width: 32),
                          Text(
                            repeatInterval[int.parse(snooze.split("|")[0])],
                            style: const TextStyle(
                                fontSize: 40, color: Colors.white),
                          ),
                          Text(
                              "Повторить сигнал ${repeatCount[int.parse(snooze.split("|")[1])]}",
                              style: const TextStyle(color: Colors.white))
                        ],
                      ),
                    ),
                  if (snooze.isNotEmpty &&
                      alarm!.snooze.split("|")[1] != "0" &&
                      !repeatState)
                    InkWell(
                      onTap: () {
                        cyrrepeatInterval = alarm!.dateTime.add(Duration(
                            minutes:
                                intervals[int.parse(snooze.split("|")[0])]));
                        if (alarm != null)
                          setAlarm(
                              Alarm(
                                  alarm!.id,
                  ...
                  ColorRoundedButton("Отключить", () async {
                    if(alarm!=null)for(var it in alarm!.offMods) {
                      switch (it) {
                        case 0:
                          {
                            final repCount = alarm!
                                        .offModsParams["WishSwipesCount"] !=
                                    null
                                ? int.tryParse(alarm!
                                        .offModsParams["WishSwipesCount"]!) ??
                                    1
                                : 1;
                            await showOverlayedMissionScreen(
                                context, repCount, 0);
                          }
                        case 1:
                          {
                            final repCount = alarm!
                                        .offModsParams["TaskSwipesCount"] !=
                                    null
                                ? int.tryParse(alarm!
                                        .offModsParams["TaskSwipesCount"]!) ??
                                    1
