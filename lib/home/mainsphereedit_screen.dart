...
class _MainSphereEditScreenState extends State<MainSphereEditScreen>
  ...
  Widget build(BuildContext context) {
    ...
    if (curWd.id == -1) {
      if (appViewModel.mainSphereEditCircle == null) {
        appViewModel.mainSphereEditCircle =
            appViewModel.mainScreenState?.allCircles[0].copy();
        appViewModel.isChanged = false;
      }
      appViewModel.isChanged = appViewModel.isChanged;
      curWd = appViewModel.mainSphereEditCircle ??
          CircleData(
              id: 0,
              prevId: -1,
              nextId: -1,
    ...
    return Consumer<AppViewModel>(builder: (context, appVM, child) {
      ...
      final aims =
          appVM.aimItems.where((element) => element.parentId == 0).toList();
      List<int> aimsids = aims.map((e) => e.id).toList();
      final tasks = appVM.taskItems
          .where((element) => aimsids.contains(element.parentId))
          .toList();
      List<MyTreeNode> root = [];
      for (var element in aims) {
        final childTasks =
            tasks.where((e) => e.parentId == element.id).toList();
        root.add(MyTreeNode(
            id: element.id,
            type: 'a',
            title: element.text,
                          ...
                          child: Column(children: [
                            if (!curWd.isActive)
                              ColorRoundedButton(
                                "Осознать",
                                () async {
                                  await appViewModel.activateSphereWish(
                                      curWd.id, true);
                                  ...
                                  showOverlayedAnimations(context,
                                      ...
                                      fillBackground: true);
                                  curWd.isActive = true;
                                  appVM.mainCircles.firstOrNull?.isActive =
                                      true;
                                },
                              ),
                                ...
                                () async {
                                  if (appVM.mainScreenState?.allCircles
                                    ...
                                    showModalBottomSheet<void>(
                                      ...
                                      builder: (BuildContext context) {
                                        return NotifyBS(
                                            'Достигнуто максимальноке количество сфер на орбите. Вы можете скрть или удалить другие сферы, чтобы освободить место для демонстрации данной сферы на орбите',
                                            "",
                                            'OK',
                                            onOk: () =>
                                                Navigator.pop(context, 'OK'));
  ...
  void showUneditable() {
    showModalBottomSheet<void>(
      ...
      builder: (BuildContext context) {
        return NotifyBS(
            "Чтобы редактировать 'Я' необходимо изменить статус на 'актуальное' нажав кнопку 'осознать'",
            "",
            'OK',
            onOk: () => Navigator.pop(context, 'OK'));
      },
